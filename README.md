...
> **Demonstration notice.** The platform uses a **Synthetic Demonstration Dataset** unless it is connected to an authorized source. Predictions, recommendations, benchmark comparisons, and reference outputs are analytical demonstrations; they are not official Government of India decisions or live PAIMANA records.
...
See [production operations](docs/production-operations.md) for the signed-gateway boundary, local configuration template, PostGIS schema, Docker service roles, coordinate publication controls, and reproducible SHAP artifact workflow.
...
| `portfolio.projects` / `portfolio.project` | Controlled project search, filters, pagination, project details, trajectory, alerts, contributors, and model prediction/explanation provenance. |
| `portfolio.alerts` / `portfolio.acknowledgeAlert` | Rule-generated warning queue, filtering, and analytical acknowledgement. |
...
| `portfolio.prediction` / `portfolio.explanation` / `portfolio.modelCard` | Governed hybrid-model outputs with source and version fields. |
...
| `portfolio.importPreview` / `portfolio.importSource` | 5 MB CSV/XLS/XLSX preview, explicit worksheet selection, project-name/sector/agency mapping, secure source-file storage, and metadata recording. |
| `briefs.generate` / `briefs.list` / `briefs.approve` | Evidence-bounded project brief creation, private archive listing, and administrator approval with audit entries. |
| `userWorkspace.publishAuthorizedCoordinate` / `userWorkspace.publishAuthorizedCoordinateBatch` | Administrator-only source-authorised coordinate publication from a manual form or a maximum-100-row CSV batch; every record requires a source, authority reference, precision, and explicit consent. |
...
See [architecture documentation](docs/architecture.md) and the in-product **Documentation** view for the problem statement, data model, feature logic, governance, setup, deployment considerations, limitations, and future extension points.
