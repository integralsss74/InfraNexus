  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (!user) {
      if (sessionUserId.startsWith("demo-")) {
        ...
        const role = parts[1] === "analyst" ? "user" : "admin";
        const name = session.name || (role === "admin" ? "MoSPI Administrator" : "Senior Risk Analyst");
        await db.upsertUser({
          openId: sessionUserId,
          name,
          email: `${parts[1]}@mospi.gov.in`,
          role,
        ...
        try {
          ...
          console.warn("[Auth] OAuth sync skipped for active session, generating local session user.");
          await db.upsertUser({
            openId: sessionUserId,
            name: session.name || "PAIMANA AI User",
            email: "user@mospi.gov.in",
            role: "admin",
            loginMethod: "Managed Session",
            lastSignedIn: signedInAt,
          });
          user = await db.getUserByOpenId(sessionUserId);
        }
